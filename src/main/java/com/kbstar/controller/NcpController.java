...
public class NcpController {
    ...
    public String ocr1impl(Model model, Ncp ncp) throws ParseException {  //Ncp에 이미지 파일덩어리가 올라와 있다
        //1.올라온 이미지를 서버에 파일 저장
        FileUploadUtil.saveFile(ncp.getImg(), imgpath); //ncp의 img에서 get, imgpath : 저장할 위치
        ...
        //String imgname = "jung.jpg";
        String imgname = ncp.getImg().getOriginalFilename(); // ncp.getImg() 이미지 덩어리에서 파일이름 가져와
        JSONObject result = (JSONObject) OCRUtil.getResult(imgpath,imgname);  //서버에 전송. result결과를 줌
        log.info(result.toJSONString());
    ...
    public String ocr2impl(Model model, Ncp ncp) throws ParseException {  //Ncp에 이미지 파일덩어리가 올라와 있다
        //1.올라온 이미지를 서버에 파일 저장
        FileUploadUtil.saveFile(ncp.getImg(), imgpath); //ncp의 img에서 get, imgpath : 저장할 위치
